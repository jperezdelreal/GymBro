...
- ViewModel tests should verify state emission for workout flows (log set → state updates → effect emitted).
- Room DAO tests should cover workout history queries and offline-first sync edge cases.
- Screenshot tests are especially important for the workout logging UI — it's the most-used screen and must be pixel-perfect.
- Use Turbine for Flow testing (test state emission sequences from ViewModels).
