...
### 5. GymBro-Specific Notes

When applying this architecture to GymBro's Android target:
- The iOS app uses MVVM + SwiftData + CloudKit. The Android counterpart should mirror the same layer separation using Kotlin equivalents.
- Domain models should be shared (or at minimum structurally identical) between iOS and Android targets where Compose Multiplatform is used.
- Workout logging, program management, and history are the MVP features — structure modules around these feature boundaries.
