...
- **Shift-left** ÔÇö Accessibility is part of the process. It needs to be considered even in prototypes or MVPs.
- **User-centric** ÔÇö Accessibility is about people. Checklists help, but the goal is not checklist compliance. The goal is to offer a great experience for users with disabilities.
- **Progress over perfection** ÔÇö Anytime is a good time to start. Focus on iterative and incremental improvements as you go. It goes a long way.
- **Test as you go** ÔÇö Manual testing is part of development.
...
- **Do not add trait names to labels** ÔÇö Say "Close", not "Close button" (VoiceOver adds "button" automatically, when using the button trait)
- **Do not use `.accessibilityHidden(true)` on interactive elements** ÔÇö Users won't be able to access them
- **Do not use fixed font sizes** ÔÇö Always use text styles for Dynamic Type support
- **Do not use hardcoded colors for text** ÔÇö Use semantic colors (`.label`, `.secondaryLabel`) for contrast and Dark Mode
- **Do not group UIKit elements without a clear combined label** ÔÇö If `isAccessibilityElement = true`, set `accessibilityLabel` (and value/traits as needed).
- **Do not group SwiftUI elements without a clear combined label** ÔÇö If `.accessibilityElement(children: .ignore)` is used, provide label/value/traits manually.
- **Do not add hints unless needed** ÔÇö It should be clear what a component expresses, and does, by its label/value/traits. Only configure for adding extra clarity or context.
- **Do not rely on `onTapGesture` alone** ÔÇö Prefer semantic controls like `Button`. If gesture handling is unavoidable, add button traits and clear labels.
- **Do not scale chrome controls with Dynamic Type** ÔÇö For navigation bars, toolbars, and tab bars, prefer Large Content Viewer (iOS 13+) using [`.accessibilityShowsLargeContentViewer`](https://developer.apple.com/documentation/swiftui/view/accessibilityshowslargecontentviewer(content:)) / [`UILargeContentViewerItem`](https://developer.apple.com/documentation/uikit/uilargecontentvieweritem).
...
- **iOS deployment target** ÔÇö Some APIs require specific versions:
...
- **Check minimum OS** ÔÇö Look for `#available` checks and deployment target in project settings

### Project Conventions
- **Design system** ÔÇö Does the project define its own design system (colors, text styles, UI component catalogue)? Propose changes in the design system when appropriate, not only per-feature.
- **Semantic colors and text styles** ÔÇö Does the project use semantic colors (`.label`, `.systemBackground`) and text styles (`.preferredFont(forTextStyle:)` in UIKit, `.font(.body)` in SwiftUI) vs hardcoded values?
- **Existing accessibility patterns** ÔÇö Search for `.accessibilityLabel`, `.accessibilityTraits`, etc. to match project style.
- **Localization** ÔÇö Accessibility labels, values, and hints should be localized. Match the project's localization conventions.
- **UI construction** ÔÇö Interface Builder (XIB/Storyboard) or code-only?
- **Custom gestures** ÔÇö Identify if custom gestures need accessible alternatives.
- **Accessibility test coverage** ÔÇö Existing UI tests auditing for accessibility?
...
**Example prompt:** ÔÇ£VoiceOver reads ÔÇÿbuttonÔÇÖ for my close button.ÔÇØ
**Expected response:**
...
**Example prompt:** ÔÇ£Dynamic Type breaks my header layout in UIKit.ÔÇØ
**Expected response:**
